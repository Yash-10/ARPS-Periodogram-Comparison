blsAndTCF <- function(
    ...
    byt <- getLightCurve(period, 0.008448276, duration, noiseType=noiseType, ntransits=ntransits, res=res, gaussStd=gaussStd, ar=ar, ma=ma, order=order)
    by <- unlist(byt[1])
    t <- unlist(byt[2])

    tyt <- getLightCurve(period, 0.013620690, duration, noiseType=noiseType, ntransits=ntransits, res=res, gaussStd=gaussStd, ar=ar, ma=ma, order=order)
    ty <- unlist(tyt[1])

    # Special case (TCF fails if absolutely no noise -- so add a very small amount of noise just to prevent any errors).
    if (noiseType == 0) {
        ty <- ty + 10^-10 * rnorm(length(ty))
    }
    ...
    boutput <- bls(by, t, bls.plot = FALSE, per.min=min(1/bfreqGrid), per.max=max(1/bfreqGrid), nper=length(bfreqGrid))
    ...
    # plot(t, y, type='l', main=sprintf("Period: %.1f days, duration: %.1f (hrs)", period, duration), cex.main=cexVal, cex.lab=cexVal, cex.axis=cexVal, xlab='time (hrs)', ylab='normalized flux')
